...
var co = require('co');
//var kv = require('../../../../../src/modules/marketing/power/kvs').power;
...
describe('Power kvs', function(){
    ...
    describe('getParticipantMapString and setParticipantMapString', function(){
        var userMap = {
            '12G1Qm':
            {
                "_id": "12G1Qm",
        ...
        it('succeed to get and set', function(done){
            ...
            co(function*(){
                try{
                    var userMapString = yield kv.getParticipantMapStringAsync(activityId);
                    if(userMapString){
                        console.info('activity\'s user map string exists:');
                        console.info(JSON.stringify(JSON.parse(userMapString), null, 4));
                        done();
                        return;
                    }

                    console.info('activity\'s user map string is BLANK, load and set it now');
                    userMapString = JSON.stringify(userMap);
                    yield kv.setParticipantMapStringAsync(activityId, userMapString);

                    userMapString = yield kv.getParticipantMapStringAsync(activityId);
                    console.info('activity\'s user map string is load and cached:');
                    console.info(JSON.stringify(JSON.parse(userMapString), null, 4));
                    done();
                }
                catch(e){
